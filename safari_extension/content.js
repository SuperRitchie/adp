...
// listener for messages from the extension. safari supports both the chrome and browser
// namespaces. we choose whichever is available at runtime to register the listener.
const extApi = typeof chrome !== 'undefined' && chrome.runtime ? chrome : (typeof browser !== 'undefined' ? browser : null);
    ...
    if (request.action === 'collectShifts') {
      ...
      (async () => {
        try {
          ...
          ics += 'END:VCALENDAR\r\n';
          // initiate download. safari requires adding the anchor to the dom before clicking.
